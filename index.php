...
if ($_SERVER["REQUEST_METHOD"]=="GET"){
    ...
    echo "<!DOCTYPE><html><head><meta charset='utf-8'><meta name='viewport' content='width=device-width, initial-scale=1.0'><link rel='stylesheet' href='index.css'><script src='index.js'></script><script src = 'https://ajax.googleapis.com/ajax/libs/jquery/3.5.1/jquery.min.js'></script><title>Financial Dashboard</title></head><body><div id='outmost'><h1>Financial Dashboard</h1><div id='container'>";    
    if (mysqli_num_rows($record)>0){
    ...
    $db_conn=mysqli_connect("mydb","dummy","c3322b","db3322") or dataBaseError();

    $current_time=time();

    $query="DELETE FROM UserRecord WHERE timestamp<".strval($current_time).";";
    $record=mysqli_query($db_conn,$query) or dataBaseError();

    if (isset($_COOKIE["userid"])==false || isset($_COOKIE["order"])==false){
        
        header("HTTP/1.1 400 Rejected the request");

    

    }
    else{
        $current_order = $_COOKIE["order"];
        if ($_COOKIE["order"]==""){
            $current_order = "\"\"";
        }
        ...
        $record=mysqli_query($db_conn,$query) or dataBaseError();
        if (mysqli_num_rows($record)==0){
            $current_user=rand(10000,99999);
            $query="SELECT * FROM UserRecord WHERE uid=".strval($current_user).";";
            $record=mysqli_query($db_conn,$query) or dataBaseError();
            while(mysqli_num_rows($record)!=0){
                $current_user=rand(10000,99999);
                $query="SELECT visible FROM UserRecord WHERE uid=".strval($current_user).";";
                $record=mysqli_query($db_conn,$query) or dataBaseError();
            }
            $current_time = time()+300;
            setcookie("userid",$current_user,$current_time);
            $query="INSERT INTO UserRecord VALUES(".strval($current_user).",".$current_order.",".strval($current_time).");";
            $record=mysqli_query($db_conn,$query) or dataBaseError();
            


        }else{
            $query="UPDATE UserRecord SET visible=".$current_order." WHERE uid=".strval($current_user).";";
            $record=mysqli_query($db_conn,$query) or dataBaseError();
        }
